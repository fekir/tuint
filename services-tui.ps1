...
function Draw-UI {
  ...
  foreach ($r in $visibleRows) {
    ...
    $state = $r.Status.ToString();
    $line = $fmt -f $marker, $r.DisplayName, $state, $r.StartType;
...
A TUI program for listing, enabling, and disabling services
...
function Get-ServiceRows {
  return @(Get-Service | Sort-Object DisplayName);
}
    ...
    while ($true) {
      ...
      switch -CaseSensitive ($key.Key) {
        ...
        "Spacebar" { # Space -> toggle status
          ...
          $item = $rows[$selected];
          try {
            $status = if ( $item.Status.ToString() -eq "Running" ) { "Stopping $($item.DisplayName)..." } else {"Starting $($item.DisplayName)..." };
            Draw-UI -buf $new -Header $header -rows $rows -selected $selected -top $top -status $status;
            Print-Diff -Old $old -New $new;
            if ($item.Status.ToString() -eq "Running") {
              Stop-Service -Name $item.Name -ErrorAction Stop | Out-Null;
              $status = "Stopped $($item.DisplayName)"
            } else {
              Start-Service -Name $item.Name -ErrorAction Stop | Out-Null;
              $status = "Started $($item.DisplayName)"
            }
            $allrows = Get-ServiceRows;
            $rows        = @($allrows | Where-Object { $_.DisplayName -match $filter });
          } catch {
        ...
        "E" {
          ...
          $item = $rows[$selected];
          try {
            Set-Service -Name $item.Name -StartupType Automatic -ErrorAction Stop | Out-Null;
        ...
        "D" {
          ...
          $item = $rows[$selected];
          try {
            Set-Service -Name $item.Name -StartupType Disabled -ErrorAction Stop | Out-Null;
        ...
        "F"  {
          ...
          $filter = $newFilter;
          $rows = @($allrows | Where-Object { $_.DisplayName -match $filter });
          $status="Filter '$filter' applied, showing $($rows.Count) scheduled tasks";
          $selected = 0;
        }
