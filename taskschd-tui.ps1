...
function Draw-UI {
  ...
  $row = $Header.Count;
  $maxName = ($rows.TaskName | Measure-Object -Maximum Length).Maximum;
  ...
  foreach ($r in $visibleRows) {
    ...
    $state = if ($r.Settings.Enabled) { "Enabled" } else { "Disabled" };
    $line = $fmt -f $marker, $r.TaskName, $state, $r.TaskPath.Replace('\', '/');
...
function Get-TaskRows {
  return @(Get-ScheduledTask | Sort-Object TaskPath, TaskName);
}
...
function Invoke-CursesUI {
  ...
  $selected      = 0;
  $top           = 0;
  $status        = "Loaded $($rows.Count) scheduled tasks";
